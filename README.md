...
A dedicated crawler agent for accessing public manga data from MangaDex. Built on `KamiYomu.CrawlerAgents.Core`, this agent enables efficient search, metadata extraction, and integration with the KamiYomu platform.
...
3. Locate and install the "KamiYomu Crawler Agent – mangadex.org" add-on.
4. Configure the add-on in the Add-ons UI as needed.

### Option B — Install as a package (for developers / extensibility)
The package is published as `KamiYomu.CrawlerAgents.MangaDex`.
...
3. Use the platform search or API to query MangaDex data. Example usage patterns:
...
- Navigate to `/src/KamiYomu.CrawlerAgents.MangaDex/`
- Double-click `KamiYomu.CrawlerAgents.MangaDex.sln`
...
- VS Code: Terminal > `dotnet build src/KamiYomu.CrawlerAgents.MangaDex/KamiYomu.CrawlerAgents.MangaDex.sln`
